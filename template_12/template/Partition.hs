...
newtype Partition a = Partition [[a]] deriving Show -- change this

-- insert Eq instance declaration here

instance (Show a, Ord a) => Eq (Partition a) where
   (==) x y = pretty x == pretty y

-- returns a list of the groups of a partition
toLists :: Ord a => Partition a -> [[a]]
toLists (Partition xs) = sort (map sort xs)
